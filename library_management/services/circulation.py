...
def renew_transaction(transaction_name: str, renewal_date=None):
    ...
    member_category = transaction.member_category or frappe.db.get_value(
        "Library Member", transaction.member, "member_category"
    )

    if not member_category:
        frappe.throw("Member Category is missing for this member.")

    policy = get_member_policy(member_category)
    if not policy:
        frappe.throw(f"No renewal policy found for member category: {member_category}")

    current_renewals = cint(transaction.renewal_count)
    max_renewals = cint(getattr(policy, "max_renewals", 0) or 0)

    # 0 or blank = unlimited renewals
    if max_renewals > 0 and current_renewals >= max_renewals:
    ...
    transaction.renewal_count = current_renewals + 1
    transaction.transaction_type = "Renew"
    transaction.last_renewal_date = renewal_date
    transaction.due_date = get_due_date(renewal_date, member_category, transaction.item)
