...
frappe.pages["library-desk"].on_page_load = function (wrapper) {
		...
		<div class="library-desk-shell">
			...
			<div class="library-desk-layout">
				<div class="library-panel library-panel--hero">
					<div class="library-kicker">${__("Circulation Desk")}</div>
					<h1 class="library-main-title">${__("Library Desk")}</h1>
					<div class="library-summary">${__("Scan a physical book first. You can also choose Issue, Renew, or Return manually.")}</div>

					<div class="library-form-grid">
						<div class="full-span library-control" data-control="scan_value"></div>
						<div class="library-control" data-control="member"></div>
						<div class="library-control" data-control="action_type"></div>
						<div class="full-span library-control" data-control="item"></div>
						<div class="library-control" data-control="condition_status"></div>
						<div class="library-control" data-control="notes"></div>
					</div>
					...
					<div class="library-action-bar">
						<button class="btn btn-primary hidden" data-action="issue">${__("Issue Copy")}</button>
						<button class="btn btn-warning hidden" data-action="renew">${__("Renew Copy")}</button>
						<button class="btn btn-secondary hidden" data-action="return">${__("Return Copy")}</button>
						<button class="btn btn-default hidden" data-action="reserve">${__("Reserve Title")}</button>
						<button class="btn btn-default" data-action="clear">${__("Clear")}</button>
					</div>
					...
					<div class="library-feedback" data-role="feedback"></div>
					<div class="library-bottom-note">
						<div class="library-muted">${__("The action dropdown is auto-filled from copy state, but you may change it manually.")}</div>
					</div>
				</div>
				...
				<div class="library-side-stack">
					...
					<div class="library-panel">
						...
						<div class="library-muted" style="margin-top: 14px;">${__("Desk prefers backend action, but action dropdown can override it.")}</div>
					</div>
				</div>
			</div>
		</div>
	...
	const controls = {
		...
		scan_value: makeControl(
			{ fieldtype: "Data", label: __("Book Barcode / Accession / Copy ID"), fieldname: "scan_value" },
			body.find("[data-control='scan_value']")
		),
		action_type: makeControl(
			{
				fieldtype: "Select",
				label: __("Action"),
			...
			body.find("[data-control='action_type']")
		),

		item: makeControl(
			{ fieldtype: "Link", label: __("Title (Optional)"), fieldname: "item", options: "Item" },
			body.find("[data-control='item']")
		),
		condition_status: makeControl(
			{
	...
	const renderFeedback = (message = "", indicator = "blue") => {
		...
		body.find("[data-role='feedback']").html(
			`<div class="alert alert-light border">${frappe.utils.escape_html(message)}</div>`
		);
	};

	const resetCopyAndAvailability = () => {
		[
			"copy_name", "item_name", "authors", "material_type",
			"copy_status", "accession_no", "current_member"
		].forEach((field) => setRoleValue(field, "-"));
		["total_copies", "available_copies", "issued_copies"].forEach((field) => setRoleValue(field, 0));
	};
	...
	const renderCopyPayload = (payload) => {
		...
		setRoleValue("copy_status", copy.status || "-");
		setRoleValue("accession_no", copy.accession_no || "-");
		setRoleValue("current_member", copy.current_member || payload?.copy_member?.name || "-");
		setRoleValue("total_copies", availability.total_copies || 0);
		setRoleValue("available_copies", availability.available_copies || 0);
		setRoleValue("issued_copies", availability.issued_copies || 0);
	...
	const renderChips = () => {
		...
		if (state.member?.status) chips.push(`Member: ${state.member.status}`);
		if (state.copyPayload?.copy?.status) chips.push(`Copy: ${state.copyPayload.copy.status}`);
		if (state.activeTransaction?.name) chips.push(`Txn: ${state.activeTransaction.name}`);
		if (state.nextAction) chips.push(`Auto: ${state.nextAction}`);
		if (getValue("action_type")) chips.push(`Selected: ${getValue("action_type")}`);
		body.find("[data-role='chips']").html(
			chips.map((chip) => `<span class="library-chip">${frappe.utils.escape_html(chip)}</span>`).join("")
		);
	};
	...
	const syncMemberStateFromCopy = () => {
		const currentMember = state.copyPayload?.copy_member?.name
			|| state.copyPayload?.copy?.current_member
			|| state.activeTransaction?.member;

		const selectedAction = (getValue("action_type") || "").toLowerCase();

		if (["return", "renew"].includes(selectedAction) && currentMember) {
			if (getValue("member") !== currentMember) {
				setValue("member", currentMember);
			}
			state.member = state.copyPayload?.copy_member || state.member;
			lockMemberField(true);
			return;
		}
	...
	const renderActions = () => {
	const hasMember = !!(getValue("member") || state.member?.name);
	const hasItem = !!(getValue("item") || state.copyPayload?.item?.name);
	const selectedAction = (getValue("action_type") || "").toLowerCase();

	setActionVisible("issue", selectedAction === "issue" && hasMember);
	setActionVisible("renew", selectedAction === "renew");
	setActionVisible("return", selectedAction === "return");
	setActionVisible("reserve", selectedAction === "reserve" && hasMember && hasItem);
	};

	const toggleConditionField = () => {
		body.find("[data-control='condition_status']")
			.closest(".library-control")
			.show();
...
const syncActionDropdownFromBackend = () => {
	if (state.syncingFields) return;

	// no scan, no item, no payload -> keep blank
	if (!state.copyPayload && !getValue("scan_value") && !getValue("item")) {
		setValue("action_type", "");
		return;
	}

	if (state.nextAction === "issue") {
		setValue("action_type", "Issue");
	} else if (state.nextAction === "renew") {
		setValue("action_type", "Renew");
	} else if (state.nextAction === "return") {
		setValue("action_type", "Return");
	} else {
		setValue("action_type", "");
	}
};
	...
	controls.action_type.$input.on("change", () => {
		if (state.syncingFields) return;
		renderActions();
		syncMemberStateFromCopy();
		renderChips();
	...
	const loadMemberContext = () => {
		...
		return frappe.call("library_management.api.get_library_member_context", { member }).then((r) => {
			state.member = r.message?.member || null;
			applyState();
		});
	};
	...
	const loadCopyContext = () => {
		...
		return frappe.call("library_management.api.barcode_lookup", { scan_value, item, member }).then((r) => {
			...
			state.copyPayload = message;
			state.copyMember = message.copy_member || null;
			state.activeTransaction = message.active_transaction || null;
			state.nextAction = message.next_action || null;

			if (["return", "renew"].includes(state.nextAction) && state.copyMember) {
				state.member = state.copyMember;
			...
			state.nextAction = null;
			applyState();
			renderFeedback(error?.message || __("Unable to load copy details."));
		});
	};
	...
	body.find("[data-action='issue']").on("click", () => {
		frappe.call("library_management.api.issue_library_copy", {
			...
			renderFeedback(__("Issued transaction {0} with due date {1}.", [r.message.name, r.message.due_date]));
			scheduleAutoRefresh();
		});
	});

body.find("[data-action='renew']").on("click", () => {
	frappe.call("library_management.api.renew_library_transaction", {
		transaction: state.activeTransaction?.name,
		scan_value: getValue("scan_value") || state.copyPayload?.copy?.name
	}).then((r) => {
		renderFeedback(__("Renewed transaction {0} with due date {1}.", [
			r.message.name,
			r.message.due_date || r.message.due_date_formatted || "-"
		]));
		scheduleAutoRefresh();
	}).catch((error) => {
		renderFeedback(error?.message || __("Unable to renew this copy."));
	});
});
	body.find("[data-action='return']").on("click", () => {
		frappe.call("library_management.api.return_library_copy", {
			...
			renderFeedback(__("Returned transaction {0} with status {1}.", [r.message.name, r.message.status]));
			scheduleAutoRefresh();
		});
	});
	...
	body.find("[data-action='reserve']").on("click", () => {
		frappe.call("library_management.api.reserve_library_item", {
			member: getValue("member") || state.member?.name,
			item: getValue("item") || state.copyPayload?.item?.name
		}).then((r) => {
			renderFeedback(__("Reservation {0} created successfully.", [r.message.name]));
			scheduleAutoRefresh();
		});
	});

	body.find("[data-action='clear']").on("click", () => {
		["member", "scan_value", "item", "notes", "action_type"].forEach((fieldname) => setValue(fieldname, ""));
		setValue("condition_status", "Good");
		state.member = null;
		state.copyMember = null;
		state.copyPayload = null;
